		...
		$.extend( Repeter.prototype, {
			...
			addItem: function(){
				...
				if (this.opt.elements.allowDuplicated) {
					...
					var itemVerify = this.opt.elements.itemVerify;

					switch (this.opt.elements.findByType) {
						case "all" :
							var rEleNum = $(this.element).find('.'+this.opt.elements.class);
							rEleNum.each(function( index, element ) {
								if ($( this ).text().indexOf(tmplData[itemVerify]) >= 0) {
									itemFound = true;
									return false;
								}
							});
							break;
						case "item":
							var itemsLoaded = $('input[id*='+this.opt.elements.tmplDataTempId+']');
							var indexItem = this.opt.elements.tmplDataTempIndex;
							itemsLoaded.each(function (index, element) {
								var item = $(this).val().split(',');
								if (tmplData[itemVerify] == item[indexItem]) {
									itemFound = true;
									return false;
								}
							});
			...
			removeItem: function(ele){
				...
				if (this.opt.elements.allowEdit) {
					...
					$.each(fields, function(index, element) {
						...
						//console.log('index = '+index+' | element = '+element+'| data = '+data[element]);
					});
				}
				//Eliminamos item
